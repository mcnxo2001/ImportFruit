...
function submitData(a, b, c, d, index) {

    arrWeight[index] = document.getElementById(a).value; //`weightFruit${idNumber}`
    arrName[index] = document.getElementById(b).value; // `fruitName${idNumber}`
    arrPerfectWeight[index] = document.getElementById(c).value; //`perfectweightFruit${idNumber}`
    amountBox[index] = document.getElementById(d).value; //`amountBoxFruit${idNumber}`
}
...
btSubmitInformation.addEventListener('click', function () {
    ...
    for (let i = 0; i < formContent.children.length; i++) {
        idNumber = parseInt(formContent.children[i].id.substr(8))
        submitData(`weightFruit${idNumber}`, `fruitName${idNumber}`, `perfectweightFruit${idNumber}`, `amountBoxFruit${idNumber}`,i)
    }
